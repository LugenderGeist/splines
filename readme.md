...
После построения сплайна он аппроксимируется точками с помощью линейной интерполяции. Полученная последовательность точек *(refined_path)* используется для управления роботом - по ней вычисляются управляющие воздействия с помощью пропорционального регулятора.

В результате удаtтся получить плавную траекторию, которая сочетает дискретный поиск пути Дейкстры с непрерывным гладким управлением. На визуализации отображается зеленый путь — исходный путь Дейкстры и красный — аппроксимированный путь, полученный с помощью сплайнов.
